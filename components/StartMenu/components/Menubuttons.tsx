...
function ContinueButton({ onClick }: { onClick: () => void }) {
  ...
  return (
    <button className={styles.startOuter} onClick={onClick}
      onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      <div className={styles.startInner}>
        {hovered && <span className={styles.startShimmer} />}
        <div className={styles.startIconWrap}><span className={styles.startIcon}>▶</span></div>
...
interface CardBtnProps {
  ...
  spinIcon?: boolean; onClick: () => void;
}

function CardButton({ icon, label, sub, accentFrom, accentTo, spinIcon, onClick }: CardBtnProps) {
  const [hovered, setHovered] = useState(false);
  return (
    <button className={styles.cardOuter} onClick={onClick}
      onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ boxShadow: hovered ? `0 8px 28px ${accentFrom}44, 0 0 40px ${accentFrom}22` : "0 2px 16px rgba(0,0,0,0.4)" }}>
      <div className={styles.cardInner}>
        <div className={styles.cardTopRow}>
          <span className={styles.cardIcon} style={{
          ...
          <div className={styles.cardBar} style={{
            background: hovered ? `linear-gradient(to right, ${accentFrom}, ${accentTo})` : "rgba(139,92,246,0.25)",
          }} />
        </div>
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <div className={styles.buttonStack}>
        {hasPlayed ? <ContinueButton onClick={onContinue} /> : <StartButton onClick={onStart} />}
        <div className={styles.secondaryRow}>
          {hasPlayed && (
            <CardButton icon="✦" label="New Game" sub=""
              accentFrom="#ec4899" accentTo="#f472b6" onClick={onStart} />
          )}
      ...
      <p className={styles.footer}>Made by Xfryy &nbsp;|&nbsp; 4th November Visual Novel</p>
    </div>
  );
}